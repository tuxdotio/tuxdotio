...
RFBClient.prototype.handleServerInit = function(data){
  ...
  console.log("Server Init, FrameBuffer Width: " + width + ", " + "Height: " + height);
  ...
  console.log("Server Init, Server Name: " + this._server_name + " (length: " + this._server_name_length + ")");
  ...
  console.log("Server Init, Bits Per Pixel: " + this._bits_per_pixel + ", Depth: " + this._depth + ", Big endian flag: " + this._big_endian_flag);
  console.log("Server Init, True Color Flag: " + this._true_color_flag + ", Red Max: " + this._red_max + ", Green Max: " + this._green_max + ", Blue Max: " + this._blue_max);
  console.log("Server Init, Red Shift: " + this._red_shift + ", Green Shift: " + this._green_shift + ", Blue Shift: " + this._blue_shift);
...
RFBClient.prototype.clientInit = function(){
  ...
  var msg = new Uint8Array(arr);
  msg[0] = 1;
  console.log("Client Init Msg: " + msg);
  this._tcpClient.send(msg);
};

RFBClient.prototype.dataReceived = function(message){
  var data = new Uint8Array(message.data);
  var decodedData = ArrayToStr(data);

  //console.log("decodedData:", decodedData, ", data:", data);
  ...
  if(!this._server_version_received){
    ...
    this.processor.dataArrived(data);
  }
};
...
var FrameBufferProcessor = function(_rfbClient) {
  ...
  this.dataArrived = function(dataIntArr){
    ...
    for (var i = 0; i < dataIntArr.length; i++) {
      this.global_buffer.push(dataIntArr[i]);
    }
    console.log('global_buffer', this.global_buffer);
...
RFBClient.prototype.handleAuthentication = function(data){
  ...
  if(this._security_type === this.VNC_AUTH_INVALID){
    ...
    console.log("VNC Authentication could not be negotiated, the reason the server gave: " + error_reason);
    this._tcpClient.disconnect(); // end the connection
    return;
  } else if(this._security_type === this.VNC_AUTH_NONE) {
    console.log("No Authentication is required...Authentication Complete");
    this._authentication_complete = true;
    this.clientInit();
    return;
  } else if(this._security_type === this.VNC_AUTH_VNCAUTHENTICATION){
    console.log("Authentication type is VNC Authentication");
    ...
    for(var i = 0; i < 16; i++){
      console.log('cca:', data.charCodeAt(i+4), data);
      if(i < 8) {
    ...
    console.log("VNC Authentication challenge: " + correctChallengelo_int + ":" + correctChallengehi_int);
    ...
    var resArr = new Uint8Array(arrBuf);
    var resHex = [];
    for(i = 0; i < 16; i++){
      if(i < 8){
    ...
    console.log('Challenge response ' + resHex);
    this._vnc_challenge_result_sent = true;
    this._tcpClient.send(resArr);
    return;
  }
};
...
RFBClient.prototype.handleServerVersion = function(data){
  if(data.substr(0,3) === 'RFB'){ /* string manipulation like this is safe for certain operations */
    ...
    console.log("Server Version RFB: " + this._server_rfb_version_string);
    console.log("Sending Client Version RFB 3.3"); // send our version to the server...
